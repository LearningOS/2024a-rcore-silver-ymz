...
use crate::{config::MAX_SYSCALL_NUM, loader::get_num_app, sync::UPSafeCell, timer::get_time_ms};
...
pub struct TaskInfoManager {
    task_infos: UPSafeCell<Vec<TaskInfo>>,
}

/// Guard for `TaskInfo`
pub struct TaskInfoGuard<'a> {
    guard: RefMut<'a, Vec<TaskInfo>>,
    task_id: usize,
}
...
lazy_static! {
    ...
    static ref TASK_INFO_MANAGER: TaskInfoManager = {
        ...
        println!("init TASK_INFO_MANAGER");
        let num_app = get_num_app();
        let mut task_infos = Vec::with_capacity(num_app);
        for _ in 0..num_app {
        ...
        TaskInfoManager {
            task_infos: unsafe { UPSafeCell::new(task_infos) },
        }
    };
}
